...
SERVICES = api-service room-service user-service code-service socket-service
...
api-service:
	docker build -t $(DOCKER_REPO)/api-service:$(TAG) ./backend/services/api-gateway
	docker push $(DOCKER_REPO)/api-service:$(TAG)
